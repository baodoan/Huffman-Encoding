...
HuffmanTree::TreeNode * HuffmanTree::removeSmallest( 
	...
	//First check if either of the Queue is empty	
	if(mergeQueue.empty())
	{
...
//std:: cout <<"Seg here" <<endl;
// First, place all leaf into singleQueue
  for(size_t i=0; i< frequencies.size(); i++)
	{
	  singleQueue.push(new TreeNode (frequencies.at(i)));
	}	

 // Next, until... one empty and another has only one left
  while(singleQueue.size() + mergeQueue.size() >1)
  {  
	small_1 = removeSmallest(singleQueue, mergeQueue); 
  	small_2 = removeSmallest(singleQueue, mergeQueue);
 	TreeNode * combine= new TreeNode(small_1->freq.getFrequency() + small_2->freq.getFrequency()); 
	 
	combine->left = small_1;
	combine->right = small_2;
...
void HuffmanTree::writeTree( TreeNode * current, BinaryFileWriter & bfile ) {
  ...
  //If it's the leaf, write the character
  if(current->left == NULL && current->right ==NULL)
{
	bfile.writeBit(1);
